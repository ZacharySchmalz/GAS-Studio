...
public class WC2
{
	public WheelCollider wheelFL2;
	public WheelCollider wheelFR2;
	public WheelCollider wheelRL2;
	public WheelCollider wheelRR2;
}
[System.Serializable]
public class WT2
{
	public Transform wheelFL2;
	public Transform wheelFR2;
	public Transform wheelRL2;
	public Transform wheelRR2;
}
[RequireComponent(typeof(AudioSource))]//needed audiosource
[RequireComponent(typeof(Rigidbody))]//needed Rigid body
public class CarControlCSNetwork : NetworkBehaviour {
    ...
    void HandleInputs()
    {
        ...
        if (!isAI && isControlActive && isLocalPlayer)
        {
            //Update wheel angle
    ...
    void AllignWheels()
	{
		...
		Quaternion quat;
		Vector3 pos;
		wheels.wheelFL2.GetWorldPose(out pos,out quat);
		tires.wheelFL2.position = pos;
		tires.wheelFL2.rotation = quat;

		wheels.wheelFR2.GetWorldPose(out pos,out quat);
		tires.wheelFR2.position = pos;
		tires.wheelFR2.rotation = quat;

		wheels.wheelRL2.GetWorldPose(out pos,out quat);
		tires.wheelRL2.position = pos;
		tires.wheelRL2.rotation = quat;

		wheels.wheelRR2.GetWorldPose(out pos,out quat);
		tires.wheelRR2.position = pos;
		tires.wheelRR2.rotation = quat;
	...
	void Drive()
	{
		...
		wheels.wheelFL2.motorTorque = maxTorque * Acceleration * gasMultiplier;
		wheels.wheelFR2.motorTorque = maxTorque * Acceleration * gasMultiplier;
		wheels.wheelRL2.motorTorque = maxTorque * Acceleration * gasMultiplier;
		wheels.wheelRR2.motorTorque = maxTorque * Acceleration * gasMultiplier;

		if (localCurrentSpeed.z < -0.1f && wheels.wheelRL2.rpm < 10) {//in local space, if the car is travelling in the direction of the -z axis, (or in reverse), reversing will be true
			reversing = true;
		} else {
			reversing = false;
		}
        ...
        //if (isAI)
       // {
            wheels.wheelFL2.steerAngle = maxSteer * Wheel;
            wheels.wheelFR2.steerAngle = maxSteer * Wheel;
       // }

		if (Brake > 0)//pressing space triggers the car's handbrake
		{
			wheels.wheelFL2.brakeTorque = handBrakeTorque * Brake;
			wheels.wheelFR2.brakeTorque = handBrakeTorque * Brake;
			wheels.wheelRL2.brakeTorque = handBrakeTorque * Brake;
			wheels.wheelRR2.brakeTorque = handBrakeTorque * Brake;
		}
		else//letting go of space disables the handbrake
		{
			wheels.wheelFL2.brakeTorque = 0f;
			wheels.wheelFR2.brakeTorque = 0f;
			wheels.wheelRL2.brakeTorque = 0f;
			wheels.wheelRR2.brakeTorque = 0f;
		}
	}
